# NoteTaker

![A screenshot of the notetaker application](https://github.com/annoyingdroid/NoteTaker/raw/main/Notetaker.png)

## Description
A web based note application that allows a user to write and save notes

## Local installation
To install this application locally, you will first need to clone this repository. Once cloned, running the command `npm run dev` will allow you to run an instance locally. From there, you will be able to access the application via localhost:3001.

## Deployment
This application is deployed via Heroku here: https://annoyingnotes.herokuapp.com

## Tests
This application contains no tests

## Tools used
* VSCode
* Node.JS
* Heroku
* Blood, sweat, and tears
